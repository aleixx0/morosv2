// index.js — MOROS BOT FINAL (con .code y .stam)
...
client.on('messageCreate', async (message) => {
  ...
  /* Auto-respuesta cuando mencionan al Owner en OFF */
  ...
  if (['.off', '.descanso'].includes(lc)) {
    if (!canControl) return message.reply('❌ Solo owner/manager/admin pueden usar `.off`.');
    ownerAway = true;
    await message.reply([
  ...
  if (['.onn', '.on', '.online'].includes(lc)) {
    if (!canControl) return message.reply('❌ Solo owner/manager/admin pueden usar `.onn`.');
    ownerAway = false;
    await message.reply([
  ...
  if (['.restart', '.reiniciar'].includes(lc)) {
    if (!canControl) return message.reply('❌ Solo owner/manager/admin.');
    try {
  ...
  if (lc.startsWith('.raidroles')) {
    ...
    if (!body) {
      await message.channel.send('⚠️ Escribe el texto. Ej: `.raidroles Reacciona con ✅ para unirte`')
        .then(m => setTimeout(() => m.delete().catch(()=>{}), 5000));
      return;
    }
    const embed = new EmbedBuilder()
  ...
  if (lc.startsWith('.wiperoles')) {
    ...
    if (!body) {
      await message.channel.send('⚠️ Escribe el texto. Ej: `.wiperoles Reacciona para rol de wipe`')
        .then(m => setTimeout(() => m.delete().catch(()=>{}), 5000));
      return;
    }
    const embed = new EmbedBuilder()
  ...
  /* .stam (nuevo) */
  if (lc.startsWith('.stam')) {
    if (!hasStaffPermission(message.member)) return;
    await message.delete().catch(()=>{});
    const body = content.slice('.stam'.length).trim();
    if (!body) {
      await message.channel.send('⚠️ Escribe el texto. Ej: `.stam Apúntate a STAM — Reacciona con ⚔️`')
        .then(m => setTimeout(() => m.delete().catch(()=>{}), 5000));
      return;
    }
    const embed = new EmbedBuilder()
  ...
  /* ===== Prefijo staff (!ban/!kick/!timeout/!alert) SOLO en canal staff ===== */
    ...
    if (cmd === 'help' || cmd === 'helpmoros') {
      await message.reply({ embeds: [new EmbedBuilder()
        ...
        .setDescription([
          ...
          '`.clear 10` — borrar últimos mensajes',
          '`.p` — número aleatorio 3–24',
        ].join('\n'))
...
client.on('interactionCreate', async (interaction) => {
  try {
    ...
    const hasPerm =
      ...
      (STAFF_ROLE_ID && m.roles?.cache?.has(STAFF_ROLE_ID)) ||
      managerRoles.some(r => m.roles?.cache?.has(r));

    if (!hasPerm) return interaction.editReply('❌ No tienes permiso para usar este comando.');

    const canal = interaction.options.getChannel('canal');
    const mensaje = interaction.options.getString('mensaje');
    if (!canal || !canal.isTextBased()) return interaction.editReply('⚠️ Ese canal no es válido o no es de texto.');
    ...
    if (!canSend) return interaction.editReply('⚠️ No tengo permisos en ese canal.');

    const embed = new EmbedBuilder()
      .setTitle('📢 Alerta del Staff')
      .setDescription(mensaje)
      .setFooter({ text: `Enviado por ${m.user.tag}` })
      .setTimestamp();

    await canal.send({ embeds: [embed] });
    await interaction.editReply(`✅ Mensaje enviado a ${canal}`);
    await logAction(m, '/alert', null, `Canal: #${canal.name} | Msg: ${mensaje}`).catch(()=>{});
  } catch (err) {
